...
export const AuthPage: React.FC = () => {
    ...
    const createCode = useQuery({queryKey: ['code'], queryFn: () => createOtpCode(phone), refetchOnWindowFocus: false, enabled: false})
    const userAuth = useQuery({queryKey: ['signin'], queryFn: () => signIn(phone, parseInt(code)), refetchOnWindowFocus: false, enabled: false})
    ...
    return(
        <div className="auth">
            ...
            {createCode.isSuccess ? <Button text='Войти' onClick={userAuth.refetch}/> : <Button text='Продолжить' onClick={sendCode}/>}
            {createCode.isSuccess ? <p onClick={sendCode} className={`${timer > 0 ? "auth__resend-text--disabled" : "auth__resend-text--enabled"} `}>Запросить код {timer > 0 ? `повторно можно через ${timer} секунд` : "" } </p> : ""}
        </div>
    )
}